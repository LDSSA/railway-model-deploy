...
Once this is done, you'll be taken to the main dashboard where you'll see your application building. Wait until it's completed, signaled by a green box saying "Success". **In this tab we can see a button that will allow us to check the logs of our app.** This will be very important to check weather our app is responding successfully to the requests sent or not.
...
You can check ralway's postgreSQL guide [here](https://docs.railway.app/databases/postgresql).
...
After waiting for the deployment to finish, after you have added a new variable, you can test that the app is connected to the database like this:
...
that there exists a `Dockerfile` this repo.
Just be sure that this file is in your repo and that they are
committed before you build your app in railway, and everything should work. If you
...
When our app is running on railway, we want to connect to a postgres database, rather than to a sqlite one.
